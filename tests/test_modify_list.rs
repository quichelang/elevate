#[test]
fn test_issue_regression_suite() {
    let source = r#"
        pub fn test_issue_1_vec_string_index() {
            let values: Vec<String> = ["a", "b"];
            let val: String = values[0];
            println!("{:?}", val);
        }
    ...
    let output = elevate::compile_source(source).expect("should compile");
    eprintln!("=== Generated Rust ===\n{}", output.rust_code);
    for note in &output.ownership_notes {
        eprintln!("  NOTE: {}", note);
    }
}
