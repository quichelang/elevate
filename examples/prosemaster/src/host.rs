...
fn start_sdl_window_runtime() -> Option<SdlMainState> {
    ...
    let window = video
        .window("Prosemaster - SDL/OpenGL Renderer", WIDTH, HEIGHT)
        .position_centered()
        .allow_highdpi()
        .opengl()
        .build()
...
fn scene_layout(width: i32, _height: i32) -> SceneLayout {
    let board_cell = if width >= 1400 { 42 } else { 38 };
    let board_x = 58;
    let board_y = 124;
    let board_size = board_cell * 15;
    let hud_x = board_x + board_size + 48;
...
fn draw_scene(buffer: &mut [u32], width: i32, height: i32, frame: &RenderPacket) {
    ...
    let board_size = layout.board_cell * 15;
    draw_shadow_panel(
        buffer,
        width,
        height,
    ...
    for row in 0..board_size {
        for col in 0..board_size {
            ...
            if label > 0 {
                ...
                draw_label(buffer, width, height, tx, ty, 3, color, &text);
            }
        }
    }

    for i in 0..=board_size {
        let thick = if i % 5 == 0 { 5 } else { 2 };
        let color = if i % 5 == 0 {
            rgb(97, 57, 31)
        } else {
            rgb(132, 91, 57)
